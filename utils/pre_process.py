...
class Normalizer:
    def __init__(self):
        ...
        self.ref_disc_center, self.ref_fovea_center = [
            99.36289978027344, 236.4633331298828
        ], [235.9381561279297, 260.9501953125]
        self.ref_image = cv2.resize(self.ref_image, (512, 512), interpolation=cv2.INTER_LINEAR)
    ...
    def detect_circle(self, image):
        ...
        if circles is not None:
            ...
            return [(circle[0], circle[1], circle[2]) for circle in circles[0, :]]  # 返回 (x, y, r)
        else:
            return []
    ...
    def move_circle_to_target(self, image, source_center, target_center):
        # 计算平移向量
        translation = ((int(target_center[0]) - int(source_center[0])), (int(target_center[1]) - int(source_center[1])))
